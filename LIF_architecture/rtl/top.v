/* module top #(
    ...
    // FIX: Calculate base_addr for the layers dynamically
    wire [$clog2(L1_IN * L1_OUT/PES)-1:0] l1_base_addr = group * L1_IN;
    wire [$clog2(L2_IN * L2_OUT/PES)-1:0] l2_base_addr = group * L2_IN;
    ...
    layer #(
        ...
        .active_pes (active_pes),       // FIX: Added active_pes mapping
        .base_addr  (l1_base_addr),     // FIX: Mapped base_addr instead of group
    ...
    layer #(
        ...
        .active_pes (active_pes),       // FIX: Added active_pes mapping
        .base_addr  (l2_base_addr),     // FIX: Mapped base_addr instead of group
    ...
    // ─── argmax — FIX: Blocking assignments for combinational loop ────
    integer j;
    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            // Use '=' so max_val updates instantly during loop unrolling
            ...
            // Output uses non-blocking since it latches to the output port
...
parameter integer DATA_WIDTH   = 8,
parameter integer ACC_WIDTH    = 16,
parameter integer L1_THRESHOLD = 255,  // <-- Split into L1
parameter integer L2_THRESHOLD = 255   // <-- Split into L2
)(
input  wire                        clk,
input  wire                        reset_n,
input  wire                        start,
...
layer #(
    ...
    .DATA_WIDTH (DATA_WIDTH),
    .ACC_WIDTH  (ACC_WIDTH),
    .THRESHOLD  (L1_THRESHOLD),      // <-- ASSIGNED TO L1
    .MEM_FILE0  ("mem/fc1_weights_bank0.mem"),
    .MEM_FILE1  ("mem/fc1_weights_bank1.mem"),
    .MEM_FILE2  ("mem/fc1_weights_bank2.mem"),
    .MEM_FILE3  ("mem/fc1_weights_bank3.mem"),
...
layer #(
    ...
    .DATA_WIDTH (DATA_WIDTH),
    .ACC_WIDTH  (ACC_WIDTH),
    .THRESHOLD  (L2_THRESHOLD),      // <-- ASSIGNED TO L2
    .MEM_FILE0  ("mem/fc2_weights_bank0.mem"),
    .MEM_FILE1  ("mem/fc2_weights_bank1.mem"),
    .MEM_FILE2  ("mem/fc2_weights_bank2.mem"),
    .MEM_FILE3  ("mem/fc2_weights_bank3.mem"),
